...
public:
    ...
    Rational(const int n = 0, const int d = 1): nominator(n), denominator(d) {
        simplify();
    }
    ...
    bool operator==(const Rational& other) const; // this
...
inline std::ostream& operator<< (std::ostream& out, const Rational& item) {
    out << item.nominator << "/" << item.denominator << std::endl;
    return out;
}

inline std::istream& operator>> (std::istream& in, Rational& item) {
